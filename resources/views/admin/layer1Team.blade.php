...
@section('content')
  <div class="w-full flex flex-col gap-6 items-center justify-center" id="appCapsule">
    ...
    <div class="card text-white p-6 md:p-8 rounded-xl">
      ...
      <div>
        <h1 class="text-2xl font-bold mb-2 text-white">My Team ({{ $users->count() }})</h1>
      </div>
      </div>
      ...
      <table class="w-full display" id="userTable">
        ...
        <a v-if="user.is_active==1" v-on:click="toggleUserStatus(user.id,user.is_active,user.fullname)"
        class="status-badge text-xs font-medium py-1 px-2 text-black rounded-full bg-green-500">
        Active</a>
        <a v-if="user.is_active==0" v-on:click="toggleUserStatus(user.id,user.is_active,user.fullname)"
